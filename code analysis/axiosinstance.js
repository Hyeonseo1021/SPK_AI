...
const axiosInstance = axios.create({
 ...
 //특정 대화에 메세지 전송
export const sendMessage = async (conversationId, messageContent, role = 'user') => {//메세지 보낼 대화의 id, 내용, 보내는 주체
  const message = {
    role: role,
    content: messageContent, //메세지 객체 생성
  };
...
export const deleteAllChats = async () => { 
    try {
      const response = await axiosInstance.delete('/chat/all-c');// chat/all-c: 모든대화 조회 or 리스트 가져옴
      return response.data;
    } catch (error) {
...
//GET 요청:서버로부터 데이터를 요청할 때 사용합니다.
//주로 정보 조회(예: 웹 페이지, 이미지, API 데이터 등)에 사용됩니다.

//POST 요청:서버에 데이터를 전송하여 새로운 리소스를 생성하거나 기존 리소스를 수정할 때 사용합니다.
//주로 폼 제출, 사용자 등록, 데이터 업로드 등에서 사용됩니다.


// 새로운 대화 시작 GET 요청 사용, 새로운 대화ID 생성
...
// 새로운 대화 시작 POST요청 사용, 초기메세지 포함하여 대화시작
...
export const fetchConversations = async () => {
  try {
    ...
    const conversations = response.data.conversations || []; //respone(응답)에서 conversations 추출
    conversations.forEach(conversation => {
    }); // 각 대화 객체에 대해 반복작업 수행, 대화정보 출력 가능
...
export const fetchMessages = async (conversationId) => {
  try {
    ...
    return response.data.conversation.chats || [];//(응답)respone에서 conversation 객체의 chat속성 추출
  } catch (error) {
...
export const createModel = async (modelName, trainingData) => {
  try {
    ...
    return response.data; //POST요청에 대한 응답데이터 반환
  } catch (error) {
...
export const getChatboxes = async () => {
  try {
    ...
    // 가장 최근의 chatbox 찾기
    // reduce 함수: 배열의 모든요소를 순회,주어진 함수를 실행하여 하나의 값으로 축소
