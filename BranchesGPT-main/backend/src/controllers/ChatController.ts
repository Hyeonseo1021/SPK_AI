...
export const generateChatCompletion = async ( // 메세지에 대한 응답을 생성하는 함수
	req: Request,
	res: Response,
	next: NextFunction
) => {
	try {
		...
		const user = await User.findById(res.locals.jwtData.id);//ID로 사용자 찾기
		if (!user) {
		...
		chats.push({ content: message, role: "user" }); //사용자 메세지 추가 
		...
		// get latest response
		const chatResponse = await openai.chat.completions.create({  // open AI에게 요청하고 응답 받기 
			model: ModelName,
			messages: chats as OpenAI.Chat.ChatCompletionMessageParam[],
		});

		// push latest response to db
		conversation.chats.push(chatResponse.choices[0].message); //응답을 저장
		await user.save();
...
export const getAllConversations = async ( // 사용자의 모든 대답을 반환하는 함수
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const deleteAllConversations = async (  // 사용자의 모든 대답을 삭제하는 함수
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const startNewConversation = async ( // 새로운 대화를 시작하는 함수
	req: Request,
	res: Response,
	next: NextFunction
) => {
...
export const startNewConversationwith = async ( //새로운 대화를 이어 받는 함수
	req: Request,
	res: Response,
	next: NextFunction
) => {
